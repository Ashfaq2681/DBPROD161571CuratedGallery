...
const Hero = () => {
  ...
  const [Placeholder, setPlaceholder] = useState(" Image...");
        ...
        <div className="flex flex-col md:flex-row-reverse justify-between md:items-center md:gap-12 lg:gap-28 max-w-[1400px] mx-auto">
          ...
          <div className="max-w-[525px]">
            ...
            <form>
              ...
              {Error && (
                <p className="bg-red-300 p-5 flex flex-row items-center gap-4 my-5  rounded-xl text-[20px]">
                  <img
                    ...
                    alt="warning"
                  />
                   bar is Empty, please type something to 
                </p>
              )}
              <input
                value={input}
              ...
              />
              {input === "" && 
              <button onClick={handle} className="hover:opacity-80 text-xl text-gray-500 font-semibold rounded-full p-1">
               <Go />
              </button>}
              {input !== "" && 
              <Link to={`/Result`} className="hover:opacity-80 text-xl text-gray-500 font-semibold rounded-full p-1" state={{ Result }} key={Result}>
                <Go />
              </Link>}
              </div>
            </form>
